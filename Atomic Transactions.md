...
In there Alice sees that the address is **NOT** in her wallet (so it must be in Bob's wallet since he's signed the transaction) and Bob does indeed have enough CHI to pay her. That output contains 50 CHI, and Alice is only selling the magic sword for 10 CHI, so all is fine so far. Later below in the transaction she'll verify where it is sent.
...
Alice can now verify the almost complete transaction as shown above, i.e.:
...
The `vin` values we've already checked. The result here so far only has Bob's `scriptSig` completed. Alice must sign it before the other one is filled in. 
...
You can run xaya-tx with the new hex value to see that the second `scriptSig` has now been signed.
